...
public final class WeatherMapper {
    ...
    public static WeatherFeedback from(WeatherFeedbackDto dto) {
        return new WeatherFeedback(
                dto.isReady(), dto.getNextRefreshAt(),
                dto.getClothing(), dto.getFood(), dto.getAccommodationNotes());
    }

    // trip_id 기반 예보 조회 응답을 변환한다 (ready=false면 days가 null일 수 있음)
    public static WeatherForecast from(WeatherForecastDto dto) {
        return new WeatherForecast(dto.isReady(), dto.getNextRefreshAt(), from(dto.getDays()));
    }
